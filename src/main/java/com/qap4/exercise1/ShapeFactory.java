...
public class ShapeFactory {
    ...
    public static CircularShapes circularFactory(String colour, boolean filled, double majorAxis, double minorAxis) {
        ...
        if (checkedMinorAxis == checkedMajorAxis) {
            ...
            return new Ellipse(colour, filled, checkedMajorAxis, checkedMinorAxis);
        }
    }

    // overloaded method to handle when the user just inputs a radius
    public static CircularShapes circularFactory(String colour, boolean filled, double radius) {
        boolean check;
        check = Helper.isValidCircularShape(radius);
        if (!check) {
            throw new IllegalArgumentException("Error invalid negative values");
        }

        return new Circle(colour, filled, radius);
    }
